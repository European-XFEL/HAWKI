          ...
          // Create footnote reference HTML
            const footnotesRef =
                ...
                .map(
                    ...
                    `<a class="inline-citation" href="javascript:void(0)" onclick="scrollToSource('source${randomId}:${idx + 1}')" >${idx + 1}</a>`
                )
                .join(', ') +
                `</span></sup>\n`;
            ...
            const safeReplace = (text, target, replacement) => {
            const regex = new RegExp(`(?<![\\*\\w])(${escapeRegExp(target)})(?![\\w])`, 'g');
            return text.replace(regex, `$1${replacement}`);
            };
            text = safeReplace(text, segmentText, `%%HTML_PRESERVED_${id}%%`);
        }
      });
    }
...
function scrollToSource(id) {
    ...
    if (el) {
      ...
      setTimeout(() => el.classList.remove('highlight-scroll-target'), 1500);
    }
  }
